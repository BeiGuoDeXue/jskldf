...
#ifndef __BSP_HPS3D_H_
#define __BSP_HPS3D_H_
int hps3d(void);
...
typedef struct
{
	uint16_t threshold_state[AREA_NUM];
	uint16_t threshold_dist_avg[AREA_NUM];
	OBS_HPS_FLAG obs_hps_flag;
}obs_hps_t;
/***********************************************************************************/
...
typedef struct
{
	...
	uint16_t dist_min_x;					/*不可用*/
	uint16_t dist_min_y;					/*不可用*/
	uint16_t threshold_dist_avg[3];			/*满足阈值的平均距离:对应 threshold_state bit0:zone0, bit1:zone1, bit2:zone2*/
